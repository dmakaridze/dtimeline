    ...
    return this
        .each(function() {
          ...
          if (action === "zoom_out") {
            if (data.currentZoom < 12) {
              ...
              var requestURL = 'http://e.gov.ge/timeline/ajax/timelinecontent/'
                  + data.currentTime + '/' + data.currentZoom;
              $.ajax(
                  {
                    url : 'http://e.gov.ge/timeline/ajax/timelinecontent/'
                        + data.currentTime + '/' + data.currentZoom
                  }).done(function(c) {
          ...
          if (action === "redraw") {
            ...
            data.timeline.style.height = data.windowHeight
            ...
            var navBarContent = data.timeline.getElementsByClassName('content')[0];
            navBarContent.style.height = data.navBarContentHeight - 50 + "px";
            data.rowHeight = (data.navBarContentHeight - 50) / 3;
            var timeMarkers = data.timeline.getElementsByClassName('marker');
            var markerTime = 0;
            ...
            data.timeWidth = (2 * data.windowWidth - 255)
            ...
            for (i = 0; i < timeMarkers.length; ++i) {
              ...
              timeMarkers[i].getElementsByClassName('flag')[0].style.top = data.rowHeight
                  * (minPoint) + 1 + "px";
              timeMarkers[i].getElementsByClassName('flag')[0].style.height = data.rowHeight
                  - 3 + "px";
            }
            ...
            var br = data.timeline.getElementsByClassName('timenav')[0]
